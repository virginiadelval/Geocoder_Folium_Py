...
def my_catastro():
    while True:      # condicion si es distinto de 0 continua. 
        if numero!=0: 
            ...
            # consulta=input(f"Puede hacer la consulta en el siguiente enlace: https://municipalidadsalta.gob.ar/ws_catastro/get_data.php?id={numero}")
...
''' Realiza una peticiona al una API que da el siguiente estructura de JSON 
[
  {
    "catastro": 4324,
    "distrito": "R1",
...
if response.status_code == 200:  #capta el error si es servidor no esta escuchando
         data = response.json()  # pideo que la restpuesta sea en un Json. El Json es un diccionario en Py
        #  print(data) # imprime el valor obtenido-> toda la info. yo solo necesito lat/long.
         json_data=data[0] # Transroma los datos en una Lista y  Obtiene el primer elemento del resultado (si hay más)
...
print(location.address) # muestra la localizacion.
...
folium.Marker(
    ...
    #  popup_style=PopupStyle)
     popup=f"Esta es el Catastro N° {numero}<br>El domicilio es: <b>Calle:</b> {calle} {numeracion},<br><b>Barrio:</b> {barrio}<br><b>País:</b> {pais}, <b>Código postal:</b> {codigopostal}",
            ...
            popup_style=PopupStyle  # Agregar el estilo PopupStyle al popup
